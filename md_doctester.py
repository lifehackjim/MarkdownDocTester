...
class MDTest():
    ...
    def md_addgen(self):
        ci = self.conf.get(self.CI, '')
        utctime = time.strftime("%c %Z")
        if ci:
            ci = "Contact info: **{}**".format(ci)
        else:
            ci = "Username: **{}**".format(getpass.getuser())
        m = (
            "###### generated by: `{} v{}`, date: {}, {}"
    ...
    def md_addsecth(self, sectname, sectdict):
        ...
        hd = int(sectconf.get(self.DEPTH, 1))
        m = (
            "{} {}\n"
        ).format(('#' * hd), sectname)
        self.md.append(m)
    ...
    def md_addvalout(self, sectname, sectdict):
        if not self.conf.get(self.VALBOOL, True):
            return
